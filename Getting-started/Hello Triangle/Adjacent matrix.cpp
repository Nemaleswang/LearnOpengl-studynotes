...
int main(){
	...
	unsigned int VBO,VAO,EBO;//需要生成一个VBO对象
	...
	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);//该函数可以绘制线框模式
	//第一个参数代表我们要应用到所有三角形的正面和背面，第二个参数用线绘制
	...
	while (!glfwWindowShouldClose(window)){//检查glfw是否要求被退出
		...
		//glDrawElements(GL_TRIANGLES,6, GL_UNSIGNED_INT, 0);//绘制一个三角形，其实索引为零，顶点数为3
